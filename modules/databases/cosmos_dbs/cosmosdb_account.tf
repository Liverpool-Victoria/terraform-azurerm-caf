...
resource "azurerm_cosmosdb_account" "cosmos_account" {
  ...
  free_tier_enabled                  = try(var.settings.free_tier_enabled, try(var.settings.enable_free_tier, false))
  ip_range_filter                    = try(var.settings.ip_range_filter, null)
  multiple_write_locations_enabled   = try(var.settings.multiple_write_locations_enabled, try(var.settings.enable_multiple_write_locations, false))
  automatic_failover_enabled         = try(var.settings.automatic_failover_enabled, try(var.settings.enable_automatic_failover, null))
